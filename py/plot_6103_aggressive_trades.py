"""6103 最有力ルール(0.5% + 前日高安手仕舞い)の売買チャートを出力する。"""
...
def main() -> None:
    ...
    csv_files = [base_dir / "data" / "6103" / "6103.T.csv"]
    cs = load_csv(csv_files)
    trades = run_backtest_with_trades(cs, datetime.date(2013, 1, 1))

    out_dir = Path("charts/6103_aggressive")
    out_dir.mkdir(parents=True, exist_ok=True)

    data_end = cs[len(cs) - 1].date
    recent_start = datetime.date(data_end.year - 1, data_end.month, data_end.day)
    periods = [
        (recent_start, data_end, "6103 0.5%+前日高安 直近1年", out_dir / "chart_1year.png"),
        (datetime.date(2013, 1, 1), datetime.date(2013, 12, 31), "6103 0.5%+前日高安 2013", out_dir / "chart_2013.png"),
        (datetime.date(2020, 1, 1), datetime.date(2020, 12, 31), "6103 0.5%+前日高安 2020", out_dir / "chart_2020.png"),
        (datetime.date(2024, 1, 1), datetime.date(2024, 12, 31), "6103 0.5%+前日高安 2024", out_dir / "chart_2024.png"),
    ]
